...
class QualityScoreAgent:
    ...
    async def __get_quality_score_batch(self, documents: list[Document]) -> list[Document]: 
        ...
        #retry with inc await time, as they fail due to rate limit 
        if documents_with_scores: 
            print(
                f"Retrying {len(documents_with_scores)} failed documents with inc await times"
            )
    ...
    async def __get_quality_score(
        ...
        async def process_documents() -> Document: 
            input_user_prompt = self.SYSTEM_PROMPT_TEMPLATE.format(
                document=document.content 
            )
            try: 
                inout_user_prompt = utils.clip_tokens(input_user_prompt,model_id=self.model_id) 
            except Exception as e: 
                print(f"Failed to clip tokens for document {document.id}: {str(e)}") 
            
            try: 
                response = await acompletion(
                    model=self.model_id,
                    messages=[
                        {"role": "user", "content": inout_user_prompt} 
                    ],
                    stream=False 
                )
                await asyncio.sleep(awaot_time_seconds) 
                if not response.choices: 
                    print("No quality score generated for document {document.id}") 
                    return document 
                ...
                print(f"Failed to score document {document.id}: {str(e)}")
