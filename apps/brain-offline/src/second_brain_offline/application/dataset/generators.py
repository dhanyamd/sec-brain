...
class SummarizationDatasetGenerator:
    ...
    def generate(self, documents: list[Document]) -> InstructDataset: 
        ...
        """
        if len(documents) < 10: 
            print("Less than 10 documents to summarize, have atleast 10 documents ")
        ...
        print(f"Num instruct dataset samples: {len(instruct_dataset_samples)}")
    ...
    def __summarize_documents(self, documents: list[Document]) -> list[Document]: 
        ...
        """
        print(f"Num documents before pregeneration filtering: {len(documents)}") 
        filtered_documents = self.filter_documents(
            self.pregeneration_filters, documents
        )
        print(f"Num documents after pregeneration filtering: {len(filtered_documents)}") 
